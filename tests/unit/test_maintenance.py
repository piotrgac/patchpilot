...
class TestMaintenanceWindow:
    ...
    def test_open_during_window(self) -> None:
        mw = self._window()
        # Saturday 23:30 UTC
        dt = datetime(2026, 7, 25, 23, 30, tzinfo=UTC)
        assert mw.is_open(dt)

    def test_closed_outside_window(self) -> None:
        mw = self._window()
        # Monday 12:00 UTC
        dt = datetime(2026, 7, 27, 12, 0, tzinfo=UTC)
    ...
    def test_window_crosses_midnight(self) -> None:
        ...
        dt = datetime(2026, 7, 26, 1, 0, tzinfo=UTC)
        assert mw.is_open(dt)
    ...
    def test_timezone_conversion(self) -> None:
        ...
        # Warsaw is UTC+2 in summer, so a clearer case:
        # Saturday 21:00 UTC = 23:00 CEST → just opened
        dt_open = datetime(2026, 7, 25, 21, 0, tzinfo=UTC)
        assert mw.is_open(dt_open)

        # Saturday 20:00 UTC = 22:00 CEST → still closed
        dt_closed = datetime(2026, 7, 25, 20, 0, tzinfo=UTC)
